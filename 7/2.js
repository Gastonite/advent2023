...
const findBetterHandKind = ({ cardCounts, initialKind: kind }) => {
  if (!cardCounts.has('J') || kind === FIVE)
    return kind

  if (kind === FOUR || kind === FULL)
    return FIVE
...
const puzzle = input => {
  ...
  const handsByKind = parseHands(input)
    .reduce((before, { hand, bid }) => {
      ...
      const cardCounts = countHandCards(hand)

      const kind = findBetterHandKind({
        cardCounts,
        initialKind: deduceHandKind(cardCounts)
      })
    ...
    .flatMap(kind => {
      ...
      return hands.sort((a, b) => {
        ...
        for (let i = 0; i < 5; i++) {
          ...
          const weightA = calculateCardWeight(a.hand[i])
          const weightB = calculateCardWeight(b.hand[i])
...
console.log('result is', puzzle(input)) //251421071
